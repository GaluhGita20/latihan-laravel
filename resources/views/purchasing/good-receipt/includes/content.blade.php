...
<div class=" flex-column-fluid">
	<form action="{{ route($routes.'.submitSave', $record->goodReceipt->id)}}" method="POST" autocomplete="off">
		<input type="hidden" name="_token" id="token" value="{{ csrf_token() }}">
		<input type="hidden" name="purchase_order_id" value="{{ $record->id }}">
		...
		<div class="container-fluid mt-5">
			<div class="row">
				<div class="col-md-12">
					<div class="card card-custom">
						<div class="card-header">
							<h3 class="card-title">Detail Good Receipt</h3>
							<div class="card-toolbar">
							</div>
						</div>
						<div class="card-body">
					...
					<div class="card card-custom">
						<div class="card-body">
							<div class="form-group row">
								<label class="col-md-2 col-form-label">{{ __('Tanggal Penerimaan') }}</label>
								<div class="col-md-10 parent-group">
									<input type="text" name="tgl_penerimaan"
									class="form-control base-plugin--datepicker" 
									@if(request()->route()->getName() == $routes.'.detail.show') disabled @endif
									@if(!empty($record->goodReceipt->tgl_penerimaan))
									value="{{ $record->goodReceipt->tgl_penerimaan->format('d/m/Y') }}"
									@endif
									data-options='@json([
										"startDate" => "", 
										"endDate"=> ""
									])'
									placeholder="{{ __('Tanggal Penerimaan') }}">
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-2 col-form-label">{{ __('Catatan') }}</label>
								<div class="col-md-10 parent-group">
									<textarea required name="catatan" @if(request()->route()->getName() == $routes.'.detail.show') disabled @endif class="form-control" placeholder="{{ __('Catatan') }}">{{ $record->goodReceipt->catatan }}</textarea>
								</div>
							</div>
							<div class="form-group row">
								...
								<div class="col-sm-10 parent-group">
									...
									@foreach ($record->goodReceipt->files($module)->where('flag', 'attachments')->get() as $file)
